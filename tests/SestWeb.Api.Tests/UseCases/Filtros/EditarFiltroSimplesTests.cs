...
using SestWeb.Api.UseCases.Filtros.EditarFiltro.FiltroSimples;
using System;
...
{
    [TestFixture]
    public class EditarFiltroSimplesTests
    {
        [Test]
        ...
        public void ControllerDeveTerAtributoRoute()
        {
            ...
            Check.That(attribute.Template).IsEqualTo("api/editar-filtro-simples");
        }

        [Test]
        public void EditarFiltroSimplesDeveTerAtributoHttpPut()
        {
        ...
        [TestCase(200)]
        [TestCase(400)]
        public void EditarFiltroSimplesDeveTerAtributoProducesResponseType(int statusCode)
        {
        ...
        [Test]
        public async Task EditarFiltroSimplesDeveRetornarCreatedEmCasoDeSucesso()
        {
            const string mnemônico = "DTC";
            const string nomePerfil = "Perfil1";

            var useCase = A.Fake<IEditarFiltroSimplesUseCase>();
            var presenter = A.Fake<Presenter>();
            var request = A.Fake<FiltroSimplesRequest>();
            ...
            A.CallTo(() => useCase.Execute(A<EditarFiltroSimplesInput>.Ignored)).Returns(EditarFiltroOutput.FiltroEditado(perfil, null, null));
        ...
        [Test]
        public async Task EditarFiltroSimplesDeveRetornarBadRequestObjectResultEmCasoDeFalha()
        {
            // Arrange
            ...
            A.CallTo(() => useCase.Execute(A<EditarFiltroSimplesInput>.Ignored)).Returns(EditarFiltroOutput.FiltroNãoEditado(mensagem));
