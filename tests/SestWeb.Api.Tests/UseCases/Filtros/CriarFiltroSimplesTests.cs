...
using SestWeb.Api.UseCases.Filtros.CriarFiltro.FiltroSimples;
using System;
...
{
    [TestFixture]
    public class CriarFiltroSimplesTests
    {
        [Test]
        ...
        public void ControllerDeveTerAtributoRoute()
        {
            ...
            Check.That(attribute.Template).IsEqualTo("api/criar-filtro-simples");
        }

        [Test]
        public void CriarFiltroSimplesDeveTerAtributoHttpPost()
        {
        ...
        [TestCase(201)]
        [TestCase(400)]
        public void CriarFiltroSimplesDeveTerAtributoProducesResponseType(int statusCode)
        {
        ...
        [Test]
        public async Task CriarFiltroSimplesDeveRetornarCreatedEmCasoDeSucesso()
        {
            const string mnemônico = "DTC";
            const string nomePerfil = "Perfil1";

            var useCase = A.Fake<ICriarFiltroSimplesUseCase>();
            ...
            A.CallTo(() => useCase.Execute(A<CriarFiltroSimplesInput>.Ignored)).Returns(CriarFiltroOutput.FiltroCriado(perfil, null));
        ...
        [Test]
        public async Task CriarFiltroSimplesDeveRetornarBadRequestObjectResultEmCasoDeFalha()
        {
            // Arrange
            ...
            A.CallTo(() => useCase.Execute(A<CriarFiltroSimplesInput>.Ignored)).Returns(CriarFiltroOutput.FiltroNãoCriado(mensagem));
