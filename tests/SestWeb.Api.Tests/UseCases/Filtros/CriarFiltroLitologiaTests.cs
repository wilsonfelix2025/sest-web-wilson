...
using SestWeb.Api.UseCases.Filtros.CriarFiltro.FiltroLitologia;
...
{
    [TestFixture]
    public class CriarFiltroLitologiaTests
    {
        [Test]
        ...
        public void ControllerDeveTerAtributoRoute()
        {
            ...
            Check.That(attribute.Template).IsEqualTo("api/criar-filtro-litologia");
        }

        [Test]
        public void CriarFiltroLitologiaDeveTerAtributoHttpPost()
        {
        ...
        [TestCase(201)]
        [TestCase(400)]
        public void CriarFiltroLitologiaDeveTerAtributoProducesResponseType(int statusCode)
        {
        ...
        [Test]
        public async Task CriarFiltroLitologiaDeveRetornarCreatedEmCasoDeSucesso()
        {
            const string mnemônico = "DTC";
            const string nomePerfil = "Perfil1";

            var useCase = A.Fake<ICriarFiltroLitologiaUseCase>();
            ...
            A.CallTo(() => useCase.Execute(A<CriarFiltroLitologiaInput>.Ignored)).Returns(CriarFiltroOutput.FiltroCriado(perfil, null));
        ...
        [Test]
        public async Task CriarFiltroLitologiaDeveRetornarBadRequestObjectResultEmCasoDeFalha()
        {
            // Arrange
            ...
            A.CallTo(() => useCase.Execute(A<CriarFiltroLitologiaInput>.Ignored)).Returns(CriarFiltroOutput.FiltroNãoCriado(mensagem));
