...
using SestWeb.Api.UseCases.Filtros.EditarFiltro.FiltroLinhaBaseFolhelho;
...
{
    [TestFixture]
    public class EditarFiltroLinhaBaseFolhelhoTests
    {
        [Test]
        ...
        public void ControllerDeveTerAtributoRoute()
        {
            ...
            Check.That(attribute.Template).IsEqualTo("api/editar-filtro-lbf");
        }

        [Test]
        public void EditarFiltroLBFDeveTerAtributoHttpPut()
        {
        ...
        [TestCase(200)]
        [TestCase(400)]
        public void EditarFiltroLBFDeveTerAtributoProducesResponseType(int statusCode)
        {
        ...
        [Test]
        public async Task EditarFiltroLBFDeveRetornarCreatedEmCasoDeSucesso()
        {
            const string mnemônico = "DTC";
            const string nomePerfil = "Perfil1";

            var useCase = A.Fake<IEditarFiltroLinhaBaseFolhelhoUseCase>();
            var presenter = A.Fake<Presenter>();
            var request = A.Fake<FiltroLinhaBaseFolhelhoRequest>();
            ...
            A.CallTo(() => useCase.Execute(A<EditarFiltroLinhaBaseFolhelhoInput>.Ignored)).Returns(EditarFiltroOutput.FiltroEditado(perfil, null, null));
        ...
        [Test]
        public async Task EditarFiltroLBFDeveRetornarBadRequestObjectResultEmCasoDeFalha()
        {
            // Arrange
            ...
            A.CallTo(() => useCase.Execute(A<EditarFiltroLinhaBaseFolhelhoInput>.Ignored)).Returns(EditarFiltroOutput.FiltroNãoEditado(mensagem));
