...
using SestWeb.Api.UseCases.Filtros.CriarFiltro.FiltroCorte;
...
{
    [TestFixture]
    public class CriarFiltroCorteTests
    {
        [Test]
        ...
        public void ControllerDeveTerAtributoRoute()
        {
            ...
            Check.That(attribute.Template).IsEqualTo("api/criar-filtro-corte");
        }

        [Test]
        public void CriarFiltroCorteDeveTerAtributoHttpPost()
        {
        ...
        [TestCase(201)]
        [TestCase(400)]
        public void CriarFiltroCorteDeveTerAtributoProducesResponseType(int statusCode)
        {
        ...
        [Test]
        public async Task CriarFiltroCorteDeveRetornarCreatedEmCasoDeSucesso()
        {
            const string mnemônico = "DTC";
            const string nomePerfil = "Perfil1";

            var useCase = A.Fake<ICriarFiltroCorteUseCase>();
            ...
            A.CallTo(() => useCase.Execute(A<CriarFiltroCorteInput>.Ignored)).Returns(CriarFiltroOutput.FiltroCriado(perfil, null));
        ...
        [Test]
        public async Task CriarFiltroCorteDeveRetornarBadRequestObjectResultEmCasoDeFalha()
        {
            // Arrange
            ...
            A.CallTo(() => useCase.Execute(A<CriarFiltroCorteInput>.Ignored)).Returns(CriarFiltroOutput.FiltroNãoCriado(mensagem));
