...
using SestWeb.Api.UseCases.Filtros.CriarFiltro.FiltroLinhaBaseFolhelho;
...
{
    [TestFixture]
    public class CriarFiltroLinhaBaseFolhelhoTests
    {
        [Test]
        ...
        public void ControllerDeveTerAtributoRoute()
        {
            ...
            Check.That(attribute.Template).IsEqualTo("api/criar-filtro-lbf");
        }

        [Test]
        public void CriarFiltroLBFDeveTerAtributoHttpPost()
        {
        ...
        [TestCase(201)]
        [TestCase(400)]
        public void CriarFiltroLBFDeveTerAtributoProducesResponseType(int statusCode)
        {
        ...
        [Test]
        public async Task CriarFiltroLBFDeveRetornarCreatedEmCasoDeSucesso()
        {
            const string mnemônico = "DTC";
            const string nomePerfil = "Perfil1";

            var useCase = A.Fake<ICriarFiltroLinhaBaseFolhelhoUseCase>();
            ...
            A.CallTo(() => useCase.Execute(A<CriarFiltroLinhaBaseFolhelhoInput>.Ignored)).Returns(CriarFiltroOutput.FiltroCriado(perfil, null));
        ...
        [Test]
        public async Task CriarFiltroLBFDeveRetornarBadRequestObjectResultEmCasoDeFalha()
        {
            // Arrange
            ...
            A.CallTo(() => useCase.Execute(A<CriarFiltroLinhaBaseFolhelhoInput>.Ignored)).Returns(CriarFiltroOutput.FiltroNãoCriado(mensagem));
