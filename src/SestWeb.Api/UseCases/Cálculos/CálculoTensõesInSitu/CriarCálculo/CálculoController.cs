...
{
    [Route("api/criar-calculo-tensoesinsitu")]
    [ApiController]
    public class CálculoController : ControllerBase
    {
        private readonly ICriarCálculoTensõesInSituUseCase _useCase;
        private readonly Presenter _presenter;

        public CálculoController(ICriarCálculoTensõesInSituUseCase useCase, Presenter presenter)
        {
            _useCase = useCase;
        ...
        public async Task<IActionResult> CriarCálculo(CriarCálculoTensõesInSituRequest request)
        {
        ...
        private CriarCálculoTensõesInSituInput PreencherInput(CriarCálculoTensõesInSituRequest request)
        {
            var input = new CriarCálculoTensõesInSituInput();
            ...
            if (request.ListaLot != null && request.ListaLot.Any())
            {
                ...
                {
                    if (!req.GradPressãoPoros.HasValue && !req.LDA.HasValue && !req.LOT.HasValue && !req.MR.HasValue && !req.PV.HasValue && !req.TVert.HasValue)
                        continue;
            ...
            if (request.FraturasTrechosVerticais != null && request.FraturasTrechosVerticais.TrechosFratura != null && request.FraturasTrechosVerticais.TrechosFratura.Any())
        ...
        private RelaçãoTensãoInput PreencheRelaçãoTensão(CriarCálculoTensõesInSituRequest request)
        {
        ...
        private FraturasTrechosVerticaisInput PreencherFraturaTrechosVerticais(CriarCálculoTensõesInSituRequest request)
        {
        ...
        private DepleçãoInput PreencherDepleção(CriarCálculoTensõesInSituRequest request)
        {
        ...
        private BreakoutInput PreencherBreakout(CriarCálculoTensõesInSituRequest request)
        {
