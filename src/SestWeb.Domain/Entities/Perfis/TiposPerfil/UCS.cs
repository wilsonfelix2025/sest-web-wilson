...
{
    public class UCS : PerfilBase, IPerfilDerived
    {
        static UCS()
        {
            PerfisFactory<UCS>.Register((nome, conversorProfundidade, litologia) => new UCS(nome, conversorProfundidade, litologia));
        ...
        [BsonConstructor]
        private UCS(string nome, IConversorProfundidade conversorProfundidade, ILitologia litologia) : base(nome, conversorProfundidade, litologia)
        {
        }

        #region Fields

        public static readonly TipoPerfil _tipoPerfil = Base.TiposPerfil.GeTipoPerfil<UCS>();

        public static readonly string _descrição = "Resistência à Compressão Uniaxial";
