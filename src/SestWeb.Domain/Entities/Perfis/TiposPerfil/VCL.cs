...
{
    public class VCL : PerfilBase, IPerfilDerived
    {
        static VCL()
        {
            PerfisFactory<VCL>.Register((nome, conversorProfundidade, litologia) => new VCL(nome, conversorProfundidade, litologia));
        ...
        [BsonConstructor]
        private VCL(string nome, IConversorProfundidade conversorProfundidade, ILitologia litologia) : base(nome, conversorProfundidade, litologia)
        {
        }

        #region Fields

        public static readonly TipoPerfil _tipoPerfil = Base.TiposPerfil.GeTipoPerfil<VCL>();

        public static readonly string _descrição = "Volume de Argila";
