...
{
    public class GCOLI : PerfilBase, IPerfilDerived
    {
        static GCOLI()
        {
            PerfisFactory<GCOLI>.Register((nome, conversorProfundidade, litologia) => new GCOLI(nome, conversorProfundidade, litologia));
        ...
        [BsonConstructor]
        private GCOLI(string nome, IConversorProfundidade conversorProfundidade, ILitologia litologia) : base(nome, conversorProfundidade, litologia)
        {
            EstiloVisual = new EstiloVisual("#10bb10", 1.0, EstiloLinha.Solid, TipoMarcador.Nenhum, "Transparent");
        }

        #region Fields

        public static readonly TipoPerfil _tipoPerfil = Base.TiposPerfil.GeTipoPerfil<GCOLI>();

        public static readonly string _descrição = "Gradiente de Colapso Inferior";
