...
{
    public class GPPI : PerfilBase, IPerfilDerived
    {
        static GPPI()
        {
            PerfisFactory<GPPI>.Register((nome, conversorProfundidade, litologia) => new GPPI(nome, conversorProfundidade, litologia));
        ...
        [BsonConstructor]
        private GPPI(string nome, IConversorProfundidade conversorProfundidade, ILitologia litologia) : base(nome, conversorProfundidade, litologia)
        {
            //para iniciar com a cor default Azul 
            EstiloVisual = new EstiloVisual("#FF0000", 1.0, EstiloLinha.Solid, TipoMarcador.Circulo, "#FF0000");
        }

        #region Fields

        public static readonly TipoPerfil _tipoPerfil = Base.TiposPerfil.GeTipoPerfil<GPPI>();

        public static readonly string _descrição = "Gradiente de Pressão de Poros Interpretado";
