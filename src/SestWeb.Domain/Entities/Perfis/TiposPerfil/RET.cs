...
{
    public class RET : PerfilBase, IPerfilDerived
    {
        static RET()
        {
            PerfisFactory<RET>.Register((nome, conversorProfundidade, litologia) => new RET(nome, conversorProfundidade, litologia));
        ...
        [BsonConstructor]
        private RET(string nome, IConversorProfundidade conversorProfundidade, ILitologia litologia) : base(nome, conversorProfundidade, litologia)
        {
        }

        #region Fields

        public static readonly TipoPerfil _tipoPerfil = Base.TiposPerfil.GeTipoPerfil<RET>();

        public static readonly string _descrição = "Relação entre tensões";
