...
{
    public class CálculoPressãoPorosEatonExpoenteDFiltrado: CálculoPressãoPoros, ICálculoPressãoPoros
    {
        public DadosReservatório DadosReservatório { get; set; }
        public bool ComDepleção { get; set; } = false;
        public CálculoPressãoPorosEatonExpoenteDFiltrado(string nome, GrupoCálculo grupoCálculo, CorrelaçãoPressãoPoros métodoCálculo, IPerfisEntrada perfisEntrada,
        ...
        {
            return PerfisEntrada.Perfis.Single(x => x.Mnemonico == TiposPerfil.GeTipoPerfil<Entities.Perfis.TiposPerfil.ExpoenteD>().Mnemônico && x.TemTrendCompactação);
        }
        ...
        protected override double CalcularGporo(double profundidadeMedida, double profundidadeVertical, double sobrecarga,
            double trend, double expoenteD, double exp, Func<double, double> getGppn, GrupoLitologico grupoLitológico,
            ConcurrentBag<Tuple<double, double>> profundidadesForaLitologia)
        {
            ...
            else
            {
                ...
                gporo = sobrecarga - ((sobrecarga - gppn) * Math.Pow(expoenteD / trend, exp));
                if (gporo < gppn)
                    gporo = gppn;
            }
        ...
        {
            if (BsonClassMap.IsClassMapRegistered(typeof(CálculoPressãoPorosEatonExpoenteDFiltrado)))
                return;

            BsonClassMap.RegisterClassMap<CálculoPressãoPorosEatonExpoenteDFiltrado>(calcPPoros =>
            {
                calcPPoros.SetDiscriminator(nameof(CálculoPressãoPorosEatonExpoenteDFiltrado));
                calcPPoros.AutoMap();
